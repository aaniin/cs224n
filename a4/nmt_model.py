...
class NMT(nn.Module):
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # enc_hiddens_proj is dimensions (b, src_len, h)
        enc_hiddens_proj = self.att_projection(enc_hiddens)
        # Y is dimension (tgt_len, b, e) 
        Y = self.model_embeddings.target(target_padded)
        # Y_t is dimension (1, b, e) 
        for Y_t in torch.split(Y,1,dim=0):
            # Y_t from dimension (1, b, e) to (b, e) 
            Y_t = torch.squeeze(Y_t,dim=0)
            # Ybar_t has dimension (b, e + h)
            Ybar_t = torch.cat( (Y_t,o_prev), dim=1)
            # Apply decoder step
            dec_state, o_t, _ = self.step(Ybar_t,dec_state,enc_hiddens,enc_hiddens_proj,enc_masks)
            # Append output and update previous output
            combined_outputs.append(o_t)
            o_prev = o_t
        # Convert list of (b, h) to tensor (tgt_len, b, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # bmm: (b, src_len, h) * (b, h, 1) -> (b, src_len, 1)
        e_t = torch.bmm( enc_hiddens_proj, dec_hidden)
        # Attention score e_t (b, src_len)
        e_t = torch.squeeze(e_t,dim=2)
        # dec_hidden (b, h, 1) -> (b, h)
    ...
    def save(self, path: str):
        ...
        # PyTorch 1.6 changes file format. Use old one with _use_new..=False
        torch.save(params, path,_use_new_zipfile_serialization=False)
