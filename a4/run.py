...
Options:
    ...
    --selective-backprop                    enable selective backprop
"""
import math
import sys
import pickle
...
from utils import read_corpus, batch_iter, Batch
    ...
    while True:
        ...
        for src_sents, tgt_sents in batch_iter(train_data,
            ...
            if train_iter % valid_niter == 0:
                ...
                dev_ppl = evaluate_ppl(model, dev_data, batch_size=train_batch_size*2)   # dev batch size can be a bit larger
...
def test_save_load_model(model, optimizer, model_save_path='test-model-save.bin', device='cuda:0'):
    # Save
    print('Test: saving model to [%s]' % model_save_path, file=sys.stderr)
    # save model
    model.save(model_save_path)
    # also save the optimizers' state
    torch.save(optimizer.state_dict(), model_save_path + '.optim')
