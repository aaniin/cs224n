...
def generate_outputs(model, source, target, vocab):
    ...
    torch.save(enc_hiddens, os.path.join('sanity_check_en_es_data','enc_hiddens.pkl'))
    torch.save(dec_init_state, os.path.join('sanity_check_en_es_data','dec_init_state.pkl'))
    torch.save(enc_masks, os.path.join('sanity_check_en_es_data','enc_masks.pkl'))
    torch.save(combined_outputs, os.path.join('sanity_check_en_es_data','combined_outputs.pkl'))
...
def question_1d_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    # Load Outputs
    enc_hiddens_target = torch.load(os.path.join('sanity_check_en_es_data','enc_hiddens.pkl'))
    dec_init_state_target = torch.load(os.path.join('sanity_check_en_es_data','dec_init_state.pkl'))
...
def question_1e_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    # Load Inputs
    dec_init_state = torch.load(os.path.join('sanity_check_en_es_data','dec_init_state.pkl'))
    enc_hiddens = torch.load(os.path.join('sanity_check_en_es_data','enc_hiddens.pkl'))
    enc_masks = torch.load(os.path.join('sanity_check_en_es_data','enc_masks.pkl'))
    target_padded = torch.load(os.path.join('sanity_check_en_es_data','target_padded.pkl'))

    # Load Outputs
    combined_outputs_target = torch.load(os.path.join('sanity_check_en_es_data','combined_outputs.pkl'))

    # Configure for Testing
    reinitialize_layers(model)
    COUNTER = [0]
    def stepFunction(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks):
       dec_state = torch.load(os.path.join('sanity_check_en_es_data','step_dec_state_{}.pkl'.format(COUNTER[0])))
       o_t = torch.load(os.path.join('sanity_check_en_es_data','step_o_t_{}.pkl'.format(COUNTER[0])))
       COUNTER[0]+=1
...
def question_1f_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    # Inputs
    Ybar_t = torch.load(os.path.join('sanity_check_en_es_data','Ybar_t.pkl'))
    dec_init_state = torch.load(os.path.join('sanity_check_en_es_data','dec_init_state.pkl'))
    enc_hiddens = torch.load(os.path.join('sanity_check_en_es_data','enc_hiddens.pkl'))
    enc_masks = torch.load(os.path.join('sanity_check_en_es_data','enc_masks.pkl'))
    enc_hiddens_proj = torch.load(os.path.join('sanity_check_en_es_data','enc_hiddens_proj.pkl'))

    # Output
    dec_state_target = torch.load(os.path.join('sanity_check_en_es_data','dec_state.pkl'))
    o_t_target = torch.load(os.path.join('sanity_check_en_es_data','o_t.pkl'))
    e_t_target = torch.load(os.path.join('sanity_check_en_es_data','e_t.pkl'))
...
def main():
    ...
    assert(torch.__version__ >= "1.0.0"), "Please update your installation of PyTorch. You have {} and you should at least have version 1.0.0".format(torch.__version__)
    ...
    train_data_src = read_corpus(os.path.join('sanity_check_en_es_data','train_sanity_check.es'), 'src')
    train_data_tgt = read_corpus(os.path.join('sanity_check_en_es_data','train_sanity_check.en'), 'tgt')
    ...
    for src_sents, tgt_sents in batch_iter(train_data, batch_size=BATCH_SIZE, shuffle=True):
        ...
        tgt_sents = tgt_sents
        break
    vocab = Vocab.load(os.path.join('sanity_check_en_es_data','vocab_sanity_check.json'))
